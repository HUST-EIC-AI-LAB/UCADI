...
class ConvBlock(nn.Sequential):
    def __init__(self, in_channels):
        ...
        self.add_module("conv_1", nn.Conv3d(in_channels, bottleneck * growth_rate, kernel_size = 1, stride = 1, padding = 0, bias = True))
        self.add_module("norm_2", norm_fn(bottleneck * growth_rate))
        self.add_module("act_2", act_fn())
        self.add_module("conv_2", nn.Conv3d(bottleneck * growth_rate, growth_rate, kernel_size = 3, stride = 1, padding = 1, bias = True))
...
class TransmitBlock(nn.Sequential):
    def __init__(self, in_channels, is_last_layer):
        ...
        if not is_last_layer:
            self.add_module("conv", nn.Conv3d(in_channels, in_channels // compression, kernel_size = 1, stride = 1, padding = 0, bias = True))
...
class DenseNet(nn.Module):
    def __init__(self):
        ...
        self.features.add_module("init_conv", nn.Conv3d(input_channels, conv_channels, kernel_size = 3, stride = 1, padding = 1, bias = True))
